...
export default function ProgressHeader({
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border-subtle bg-white shadow-nav">
      <nav aria-label="Main navigation">
        {/*
          ↓ Change py-3 here to resize the header height (e.g. py-2 = smaller, py-5 = taller)
        */}
        <div className="mx-auto grid w-full max-w-container-max grid-cols-3 items-center px-4 py-2 sm:px-8 lg:px-16">
          <a
            href="#main-content"
          ...
          {/* Steps — centre column, always truly centred */}
          <div className="flex justify-center">
            {showProgress && (
              <div className="flex w-44 items-center gap-2 sm:w-92 sm:gap-3">
                {steps.map((step, i) => {
