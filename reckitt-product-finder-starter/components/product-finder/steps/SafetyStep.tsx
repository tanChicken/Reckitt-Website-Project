      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
        ...
        <div className="grid gap-3 content-start">
          {safetyCards.map(({ icon, title, text }) => (
            <div
              key={title}
              className="flex gap-4 rounded-xl border border-border-subtle bg-white p-4 items-center"
            >
              {/* ── PART 2: Modified Icon Container ── */}
              {/* Added overflow-hidden to keep image constrained */}
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-surface-container-low overflow-hidden">
                {/* ── Changed from text/SVG to Next.js Image ── */}
                <Image
                  src={icon}
                  alt={`${title} icon`}
                  width={20} // Sized to 20px for good buffer in 40px container
                  height={20}
